...
class OnlineDPOTrainerSumm(BaseTrainer):
    r"""
    ...
    def __init__(
        ...
        self.summ_count = defaultdict(int)
        self.faith_count = defaultdict(int)
        self.summ_reward_sum = defaultdict(lambda: torch.zeros(1))
        self.faith_reward_sum = defaultdict(lambda: torch.zeros(1))
    ...
    def training_step(
        ...
        self.summ_count[int((self.weights["summ"] * 100).item())] += 1
        self.faith_count[int((self.weights["faith"] * 100).item())] += 1
        ...
        w_summ_key = int((self.weights["summ"] * 100).item())
        w_faith_key = int((self.weights["faith"] * 100).item())
        self.summ_reward_sum[w_summ_key] += torch.nan_to_num(chosen_rewards_per_func.detach(), nan=0.0).sum(dim=0).cpu()[0]
        self.faith_reward_sum[w_faith_key] += torch.nan_to_num(chosen_rewards_per_func.detach(), nan=0.0).sum(dim=0).cpu()[1]
        # count chosen samples for that weight bucket
        self.summ_count[w_summ_key] += int(chosen_rewards_per_func.size(0))
        self.faith_count[w_faith_key] += int(chosen_rewards_per_func.size(0))
    ...
    def _maybe_log_save_evaluate(
        ...
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            if hasattr(self, "summ_count") and hasattr(self, "faith_count") and hasattr(self, "summ_reward_sum") and hasattr(self, "faith_reward_sum") and hasattr(self, "weights"):
                # for key, cnt in self.help_count.items():
                #     logs[f"weights/count/{key/100.0}"] = float(cnt)
                for key in self.summ_count.keys():
                    logs[f"weights/summ_count/{key/100.0}"] = float(self.summ_count[key])
                for key in self.faith_count.keys():
                    logs[f"weights/faith_count/{key/100.0}"] = float(self.faith_count[key])
                for key in self.summ_reward_sum.keys():
                    logs[f"weights/summ_reward_sum/{key/100.0}"] = float(self.summ_reward_sum[key] / max(1, self.summ_count[key]))
                for key in self.faith_reward_sum.keys():
                    logs[f"weights/faith_reward_sum/{key/100.0}"] = float(self.faith_reward_sum[key] / max(1, self.faith_count[key]))
